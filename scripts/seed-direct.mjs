...
async function seedDataDirect() {
  console.log('🌱 Démarrage du seed avec insertion SQL directe...\n');
  ...
  try {
    ...
    for (const agencyData of agencies) {
      ...
      // Utiliser l'API client pour insertion avec auth bypass
      const { data: agency, error: agencyError } = await supabase
        .from('agencies')
        .insert({
          name: agencyInsert.name,
          phone: agencyInsert.phone,
          email: agencyInsert.email,
          address: agencyInsert.address,
          ninea: agencyInsert.ninea,
          plan: agencyInsert.plan,
          status: agencyInsert.status,
      ...
      if (agencyError) {
        console.error(`❌ Erreur création agence: ${agencyError.message}`);
        continue;
      }
      ...
      const bailleursData = [];
      for (let i = 0; i < config.bailleurs; i++) {
      ...
      if (bailleursError) {
        console.error(`❌ Erreur bailleurs: ${bailleursError.message}`);
        continue;
      }
      ...
      const immeublesData = [];
      for (let i = 0; i < config.immeubles; i++) {
      ...
      if (immeublesError) {
        console.error(`❌ Erreur immeubles: ${immeublesError.message}`);
        continue;
      }
      ...
      if (unitesError) {
        console.error(`❌ Erreur unités: ${unitesError.message}`);
        continue;
      }
      ...
      if (locatairesError) {
        console.error(`❌ Erreur locataires: ${locatairesError.message}`);
        continue;
      }
      ...
      for (let i = 0; i < Math.min(config.contrats, availableUnites.length, availableLocataires.length); i++) {
      ...
      if (contratsError) {
        console.error(`❌ Erreur contrats: ${contratsError.message}`);
        continue;
      }
      console.log(`  ✅ ${contrats.length} contrats créés`);

      // Mise à jour statut unités
      ...
      for (const contrat of contrats) {
        ...
        for (let mois = 0; mois < config.mois; mois++) {
          ...
          if (aPaye) {
            ...
            datePaiement.setDate(randomInt(1, 15));
      ...
      for (let i = 0; i < paiementsData.length; i += batchSize) {
        ...
        if (paiementsError) {
          console.error(`❌ Erreur paiements batch ${i}: ${paiementsError.message}`);
        }
      }
      ...
      for (let i = 0; i < config.depenses; i++) {
      ...
      if (depensesError) {
        console.error(`❌ Erreur dépenses: ${depensesError.message}`);
      } else {
