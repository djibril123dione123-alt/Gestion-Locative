...
export function DocumentScanner() {
  ...
  return (
    <div className="sk-mobile-page space-y-5">
      <div className="sk-mobile-hero overflow-hidden bg-gradient-to-br from-emerald-950 via-emerald-900 to-slate-950 p-4 text-white shadow-2xl shadow-emerald-950/15 sm:p-6">
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-4 text-2xl font-extrabold tracking-tight sm:text-4xl">Scanner un document</h1>
            <p className="mt-2 max-w-xl text-sm font-medium leading-6 text-emerald-50/75 sm:text-base">
              Scannez le QR d’une quittance, facture, contrat, mandat ou rapport pour confirmer son authenticité sans quitter l’application.
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.08] p-4 text-sm font-semibold text-emerald-50/75 backdrop-blur lg:max-w-xs">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <section className="space-y-4">
          <div className="sk-premium-panel overflow-hidden p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.12em] text-emerald-700">Caméra</p>
                <h2 className="mt-1 text-xl font-black text-slate-950">Scan QR sécurisé</h2>
                <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">{cameraMessage}</p>
              </div>
              <QrCode className="h-6 w-6 text-emerald-800" />
            </div>

            <div className="relative mt-4 aspect-[4/3] overflow-hidden rounded-[1.35rem] border border-emerald-950/10 bg-slate-950">
              ...
              {cameraStatus === 'active' && (
                <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                  <div className="relative h-48 w-48 rounded-3xl border-2 border-emerald-200 shadow-[0_0_0_999px_rgba(2,6,23,0.35)]">
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row">
              {cameraStatus === 'active' ? (
                <button type="button" onClick={stopCamera} className="sk-action sk-action-secondary justify-center">
                  <CameraOff className="h-4 w-4" />
                  Arrêter
                </button>
              ) : (
                <button type="button" onClick={startCamera} className="sk-action sk-action-primary justify-center">
          ...
          <form
            className="sk-premium-panel p-4"
            onSubmit={(event) => {
          ...
          >
            <div className="flex items-start gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-800 ring-1 ring-emerald-950/10">
                <Keyboard className="h-5 w-5" />
              </div>
              <div>
                <p className="text-xs font-black uppercase tracking-[0.12em] text-emerald-700">Saisie manuelle</p>
                <h2 className="mt-1 text-lg font-black text-slate-950">Coller un QR ou une référence</h2>
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row">
              <input
                ...
                className="min-h-12 flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold outline-none transition focus:border-emerald-700 focus:ring-4 focus:ring-emerald-900/10"
              />
        ...
        <section>
          {result || verifying ? (
            ...
            <div className="flex min-h-[24rem] flex-col items-center justify-center rounded-[1.7rem] border border-dashed border-emerald-200 bg-white/85 p-6 text-center shadow-sm">
              <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-emerald-50 text-emerald-800 ring-1 ring-emerald-950/10">
                <ShieldCheck className="h-8 w-8" />
              </div>
              <h2 className="mt-5 text-xl font-black text-slate-950">Résultat de vérification</h2>
