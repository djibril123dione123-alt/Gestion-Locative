  ...
  return (
    <div className="fixed bottom-[calc(var(--sk-mobile-bottom-nav-height)+env(safe-area-inset-bottom)+0.75rem)] right-3 z-40 flex flex-col items-end gap-2 lg:bottom-4 lg:right-4">
      ...
      {expanded && restoreStep === 'idle' && (
        <div className="bg-[#fffdf8]/60 backdrop-blur-xl border border-white/90 shadow-[0_8px_32px_0_rgba(20,40,30,0.08)] rounded-2xl p-3 w-56 space-y-2 animate-slideUp">
          <p className="text-xs font-semibold text-slate-800">Gestion des données</p>
          {lastTs && (
            <p className="text-[0.65rem] text-slate-500">Dernière sauvegarde : {relativeTime(lastTs)}</p>
          )}
          {localPending > 0 && (
            <p className="text-[0.65rem] text-action-700 font-bold leading-tight">
              {localPending} action{localPending > 1 ? 's' : ''} en attente de synchronisation
            </p>
          )}
          {restoreError && (
            <p className="text-[0.65rem] text-red-600 font-medium">{restoreError}</p>
          )}
          <div className="space-y-1.5">
            <button
              onClick={handleDownload}
              disabled={downloading}
              className="w-full flex items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[0.7rem] font-bold bg-brand-50 hover:bg-brand-100 text-brand-900 transition disabled:opacity-50"
            >
              {downloading
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[0.7rem] font-bold bg-white hover:bg-slate-50 text-slate-800 border border-slate-200 transition"
            >
              <Upload className="w-3.5 h-3.5 flex-shrink-0" />
              Restaurer
            </button>
          </div>
          <p className="text-[0.6rem] leading-tight text-slate-400 text-center">Les données sont sauvegardées automatiquement</p>
        </div>
      )}

      {restoreStep === 'preview' && restorePreview && (
        <div className="bg-[#fffdf8]/60 backdrop-blur-xl border border-white/90 shadow-[0_8px_32px_0_rgba(20,40,30,0.08)] rounded-2xl p-3 w-64 space-y-2.5 animate-slideUp">
          <div className="flex items-start justify-between">
            <p className="text-xs font-semibold text-slate-800">Aperçu de la sauvegarde</p>
          ...
          <div className="bg-slate-50 rounded-lg p-2 space-y-1">
            {restorePreview.tables.map((t) => (
              <div key={t} className="flex justify-between text-[0.65rem]">
          ...
          {restorePreview.hasPendingMutations && (
            <div className="flex items-start gap-1.5 p-1.5 bg-amber-50 border border-amber-200 rounded-lg text-[0.65rem] text-amber-800 leading-tight">
          ...
          <div>
            <p className="text-[0.65rem] font-medium text-slate-700 mb-1">Stratégie</p>
            <div className="space-y-1">
              <label className="flex items-start gap-1.5 cursor-pointer">
                ...
                />
                <div>
                  <p className="text-[0.65rem] font-medium text-slate-700 leading-tight">Fusionner (recommandé)</p>
                  <p className="text-[0.55rem] text-slate-500 leading-tight">Conserve données, ajoute nouveautés.</p>
                </div>
              </label>
              <label className="flex items-start gap-1.5 cursor-pointer">
                <input
                ...
                />
                <div>
                  <p className="text-[0.65rem] font-medium text-slate-700 leading-tight">Remplacer</p>
                  <p className="text-[0.55rem] text-red-500 leading-tight">Écrase vos données locales.</p>
                </div>
              </label>
            </div>
          </div>
          ...
          <div className="flex gap-1.5 pt-1">
            <button
              ...
              className="flex-1 px-2.5 py-1.5 rounded-lg text-[0.65rem] font-medium bg-slate-100 hover:bg-slate-200 text-slate-700 transition"
            >
              Annuler
            </button>
            <button
              onClick={handleRestoreConfirm}
              className="flex-1 px-2.5 py-1.5 rounded-lg text-[0.65rem] font-bold bg-action-600 hover:bg-action-700 text-white transition"
            >
              Restaurer
            </button>
          </div>
      ...
      {restoreStep === 'restoring' && (
        <div className="bg-[#fffdf8]/60 backdrop-blur-xl border border-white/90 shadow-[0_8px_32px_0_rgba(20,40,30,0.08)] rounded-2xl p-3 w-56 flex items-center gap-2 animate-slideUp">
          <RefreshCw className="w-3.5 h-3.5 animate-spin text-brand-700 flex-shrink-0" />
          <p className="text-xs font-medium text-slate-700">Restauration en cours…</p>
        </div>
      )}
