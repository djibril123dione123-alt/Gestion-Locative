...
declare
  ...
  v_property_names text[] := array[
    'Residence des Almadies','Immeuble Keur Yoff','Villa Mermoz Horizon','Residence Fann Baobab',
    'Immeuble Mixte Point E','Keur Mamelles','Residence Liberté Six','Centre Ouest Foire',
...
begin
  ...
  for i in 1..18 loop
    ...
    insert into public.evenements (
      ...
      (array['rendez_vous','autre','autre','contrat','rendez_vous','intervention'])[((i-1)%6)+1],
      (current_date + ((i-7)||' days')::interval)::date,'09:30',v_property_id,
      'Evenement de demonstration planifie et rattache au portefeuille.',
      case when i%3=0 then '1_jour' else 'aucun' end,v_user_ids[((i-1)%8)+1],
