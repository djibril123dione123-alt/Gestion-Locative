...
export const SYSTEM_BLOCK_PUBLIC_LABELS: Record<DocumentSystemBlockKey, string> = {
  payment_summary: 'Résumé du règlement',
  tenant_identity: 'Locataire',
  property_identity: 'Bien et unité',
  payment_breakdown: 'Détail financier',
  owner_summary: 'Synthèse bailleur',
...
export const TEMPLATE_TAG_CATEGORY_LABELS: Record<TemplateTagDefinition['category'], string> = {
...
export function getSystemBlockPublicLabel(systemKey?: DocumentSystemBlockKey | null, fallback = 'Donnée documentaire') {
...
export const TEMPLATE_TAG_CATALOG: TemplateTagDefinition[] = [
  { key: 'agency_name', label: "Nom de l'organisation", category: 'organisation', example: 'Keur Gestion', documentTypes: types('contrat', 'mandat', 'quittance', 'rapport_bailleur', 'rapport_proprietaire') },
  { key: 'agency_address', label: 'Adresse', category: 'organisation', example: 'Médina, Dakar', documentTypes: types('contrat', 'mandat', 'quittance', 'rapport_bailleur', 'rapport_proprietaire') },
  ...
  { key: 'locataire_prenom', label: 'Prénom du locataire', category: 'locataire', example: 'Fatou', documentTypes: types('contrat', 'quittance') },
  { key: 'locataire_nom', label: 'Nom du locataire', category: 'locataire', example: 'Sarr', documentTypes: types('contrat', 'quittance') },
  ...
  { key: 'loyer_mensuel', label: 'Loyer mensuel', category: 'finance', example: '300 000 F CFA', documentTypes: types('contrat', 'quittance') },
  ...
  { key: 'date_du_jour', label: "Date d'émission", category: 'date', example: '09/07/2026', documentTypes: types('contrat', 'mandat', 'quittance', 'rapport_bailleur', 'rapport_proprietaire') },
...
const contractBlocks: DocumentTemplateBlock[] = [
  ...
  block('conge', 'Congé', "Le congé est signifié par écrit avec preuve de réception. Le locataire respecte un préavis fixé d'accord parties, selon les modalités et délais légaux en vigueur, à compter de sa réception.", 3),
  ...
  block('obligations-locataire', 'Obligations du locataire', "Le locataire paie le loyer et les charges aux termes convenus, entretient les lieux, répond des dégradations, signale sans délai toute anomalie, respecte le règlement de l'immeuble et ne sous-loue pas sans autorisation écrite.", 6),
  ...
  block('penalites', 'Pénalités et recouvrement', "{{mention_penalites}}\nDes pénalités journalières de {{penalite_montant}} pourront être appliquées pendant un délai de {{penalite_delai}} jours en cas de retard. {{mention_litige}}\n{{mention_tribunal}}", 9),
  ...
  block('caution-sortie', 'Caution et remise en état', "À la sortie, les sommes justifiées au titre des réparations locatives, consommations ou impayés pourront être retenues sur la garantie, sur présentation de justificatifs.", 11),
  ...
  block('frais', 'Frais et procédure', "{{mention_frais_huissier}} Les frais justifiés de procédure pourront être mis à la charge de la partie défaillante. Le montant indicatif configuré est de {{frais_huissier}}.", 13),
...
const mandateBlocks: DocumentTemplateBlock[] = [
  ...
  block('pouvoirs', 'Pouvoirs confiés', "Le mandataire peut rédiger les actes de location, réaliser les états des lieux, percevoir les loyers, et engager les diligences utiles. Le mandataire est autorisé à engager l'entretien courant et les réparations jusqu'à la limite convenue d'accord parties. Au-delà, l'approbation du mandant est requise, sauf action d'urgence conservatoire nécessitant une intervention immédiate (avec obligation de reporting au bailleur).", 5),
  ...
  block('mentions', 'Recouvrement et litiges', '{{mention_penalites}}\nLes frais justifiés de procédure pourront être mis à la charge de la partie défaillante.\n{{mention_tribunal}}', 7),
